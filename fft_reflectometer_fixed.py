#!/usr/bin/env python3
"""OFDR analysis of a wavelength sweep (EXFO JSON).

Corrected version. Key differences from the old version are documented
in CHANGES.md.
"""
...
N_GROUP_DEFAULT = 1.468          # SMF-28 @1550 nm, group refractive index
...
def clean(wl: np.ndarray, chans: dict[str, np.ndarray]) -> tuple[np.ndarray, dict[str, np.ndarray]]:
    """Remove non-monotonic outliers (EXFO appends a jump back to the start wavelength
    at the end -- this corrupts mean(diff))."""
    ...
    for i in bad:
        keep[i + 1] = False
    if (~keep).any():
        print(f"  [clean] {int((~keep).sum())} outliers on wavelength axis discarded")
...
def balanced(chans: dict[str, np.ndarray], channel: str | None) -> tuple[str, np.ndarray]:
    ...
    This exactly removes the laser power envelope instead of approximately,
    ...
    if len(ports) >= 2:
        ...
        if np.corrcoef(a, b)[0, 1] < -0.3:          # truly anti-phase?
...
def reflectogram(wl: np.ndarray, y: np.ndarray, n_g: float, zero_pad: int = 4):
    """FFT against 1/lambda -> peak frequency is directly n_g * OPD.

    The device sampling grid is uniform in lambda; reported wavelengths are rounded
    to 1 pm. For phase calculation, an ideal uniform grid is a better assumption
    than the rounded values.
    """
...
def plot(path: Path, lam, opd_m, spec, lim, n_g, round_trip: bool, ch: str):
    ...
    ax.set_title(f"Reflectogram — {ch}   ·   Resolution {lim['resolution_um']:.0f} µm",
...
def process(path: Path, channel: str | None, n_g: float, out: Path, round_trip: bool):
    ...
    if frac > 0.25 or d_peak > 0.8 * lim["opd_max_m"] * scale:
        print(f"\n  !!! WARNING: {frac*100:.0f} % of energy lies in the top 20 % "
              f"before the Nyquist limit.\n"
              f"      This is the signature of aliasing. The displayed length is"
              f" probably WRONG.\n"
              f"      Remedy: reduce step size delta-lambda"
              f" (halve -> double range).\n"
              f"      Check: record a sweep with half delta-lambda."
              f" If peak moves, it was aliased.\n")
...
def main():
    ap = argparse.ArgumentParser(description="OFDR analysis of an EXFO wavelength sweep.")
    ap.add_argument("filename", nargs="?")
    ap.add_argument("--channel", help="force single channel instead of balanced detection")
    ap.add_argument("--group-index", type=float, default=N_GROUP_DEFAULT)
    ap.add_argument("--single-pass", action="store_true",
                    help="Arms pass through once (no circulator/reflection) "
                         "-> x-axis is arm length difference instead of reflector distance")
