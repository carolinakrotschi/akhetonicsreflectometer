...
**And the good news:** once the aux MZI sits on its own channels (Ch1/Ch2),
the problem disappears structurally. Then the transmission signal is no
longer on the measurement channels at all, and the axis on Ch3/Ch4 is
...
2. **Check which channels are aux and which are measurement.** It's stated
   as an assumption in the file header: Ch1/Ch2 aux, Ch3/Ch4 measurement
   (confirmed 2026-08-18 against the real wiring — this had been the other
   way around in an earlier draft, so it was worth exactly this kind of
   double-check).
...
together, both outputs onto Ch1/Ch2. Lay the long spool somewhere thermally
