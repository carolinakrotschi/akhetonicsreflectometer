...
```
        ...
        │         ├────────► Ch3   ┐ AUX-MZI = your ruler
        │         └────────► Ch4   ┘ (complementary outputs)
        │
   20 dB Coupler ──────► LO arm ──┐
    ...
    Circulator ──► TEST FIBER      │
        ...
        │                          │
        └──────────────► Coupler ──┘
                          ├────────► Ch1   ┐ MEASUREMENT interferometer
                          └────────► Ch2   ┘ (as before)
```

**Why 4 channels:** Ch1/Ch2 = the two complementary outputs of your
existing measurement. Ch3/Ch4 = the two complementary outputs of the new
Aux-MZI. For both pairs you perform balanced subtraction (see A.5, step
...
9. Before every pipeline change: test on synthetic data with known ground
   truth (`tools/simulate_scan_data.py`).
